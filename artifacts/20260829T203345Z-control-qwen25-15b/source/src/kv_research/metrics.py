...
def control_summary(records, native_cache_nmse_limit, identity_continuation_bound="machine_floor"):
    if identity_continuation_bound not in ("machine_floor", "native_execution_dominance"):
        raise RuntimeError(f"unknown identity continuation bound {identity_continuation_bound}")
    ...
    for record in records:
        ...
        for layers_per_target, identity in record["identity_translation"].items():
            ...
            if identity_continuation_bound == "machine_floor":
                if identity["continuation_nmse"] > machine_limit:
                    failures.append(
                ...
                cache_limit = min(
                    ...
                    max(
                        machine_limit,
                        record["replay_vs_rebuilt_cache_nmse"],
                        record["full_vs_replay_cache_nmse"],
                    ),
                )
                logit_js_limit = max(
                    torch.finfo(torch.bfloat16).eps ** 4,
                    record["full_vs_replay"]["js"],
                    record["replay_vs_rebuilt"]["js"],
                )
                if identity["effective_cache_nmse_limit"] != cache_limit:
                    failures.append(
                        "identity cache limit was recorded incorrectly at "
                        f"{length}/{replay_length}/k{layers_per_target}"
                    )
                if identity["effective_logit_js_limit"] != logit_js_limit:
                    failures.append(
                        "identity logit limit was recorded incorrectly at "
                        f"{length}/{replay_length}/k{layers_per_target}"
                    )
                if identity["suffix_nmse"] > cache_limit:
                    failures.append(
                        "identity suffix exceeded native execution variation at "
                        f"{length}/{replay_length}/k{layers_per_target}"
                    )
                if identity["logits"]["js"] > logit_js_limit:
                    failures.append(
                        "identity logits exceeded native execution variation at "
                        f"{length}/{replay_length}/k{layers_per_target}"
                    )
    ...
    if identity_continuation_bound == "native_execution_dominance":
        summary.update(
            {
                "identity_continuation_bound": identity_continuation_bound,
                "max_identity_suffix_nmse": max(
                    identity["suffix_nmse"]
                    for row in records
                    for identity in row["identity_translation"].values()
                ),
                "max_identity_logit_js": max(
                    identity["logits"]["js"]
                    for row in records
                    for identity in row["identity_translation"].values()
                ),
                "max_identity_cache_limit_fraction": max(
                    identity["suffix_nmse"] / identity["effective_cache_nmse_limit"]
                    for row in records
                    for identity in row["identity_translation"].values()
                ),
            }
