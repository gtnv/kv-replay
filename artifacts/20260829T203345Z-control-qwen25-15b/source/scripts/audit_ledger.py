...
for entry in entries:
    ...
    if "audit_file" in entry:
        ...
        if audit.get("manifest_sha256") != entry["root_sha256"]:
            raise RuntimeError(f"ledger line {line_number} audit targets a different manifest")

    if entry["outcome"] in ("success", "failure"):
        for name in ("audit_file", "audit_sha256", "protocol_file", "protocol_sha256"):
            if name not in entry:
                raise RuntimeError(f"ledger line {line_number} audited result lacks {name}")
        if entry["root_file"] != "manifest.json":
            raise RuntimeError(f"ledger line {line_number} audited result lacks a manifest root")
        summary = read_json(artifact_dir / "summary.json")
        control_result = "passed" in summary
        if control_result and not isinstance(summary["passed"], bool):
            raise RuntimeError(f"ledger line {line_number} control passed field is not boolean")
        expected_status = "complete"
        if control_result:
            expected_outcome = "success" if summary["passed"] else "failure"
            if entry["outcome"] != expected_outcome:
                raise RuntimeError(
                    f"ledger line {line_number} control summary requires outcome "
                    f"{expected_outcome}, found {entry['outcome']}"
                )
            expected_status = "complete" if summary["passed"] else "failed_control"
        if root["status"] != expected_status:
            raise RuntimeError(
                f"ledger line {line_number} outcome {entry['outcome']} requires manifest status "
                f"{expected_status}, found {root['status']}"
            )
        omitted = [
            relative
