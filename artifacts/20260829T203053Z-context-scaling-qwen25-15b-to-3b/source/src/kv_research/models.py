...
def fixed_control_tokens(tokenizer, length, device, variant):
    if variant == 1:
        text = (
            "A systems agent inspected a failed database migration. The transaction was rolled "
            "back, the schema version remained unchanged, and the next action must preserve user "
            "data. The observation includes a retry token, a timestamp, and an explicit "
            "instruction to avoid the destructive fallback. "
        )
    elif variant == 2:
        text = (
            "A research worker compared two cached execution paths after a service handoff. The "
            "first trace retained the original prefix, while the second rebuilt state before "
            "replaying the same observation. Every token, position, and next action was recorded "
            "before deciding whether the paths were numerically equivalent. "
        )
    else:
        raise RuntimeError(f"unknown fixed control text variant {variant}")
