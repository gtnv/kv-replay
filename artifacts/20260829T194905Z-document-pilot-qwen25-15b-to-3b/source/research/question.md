...
The frozen primary comparison is 64-token commutator JS versus transferred-state negative top-1 margin. Replay length 65 is a target-forward-token-matched repair baseline; complete wall and CUDA latency remain authoritative because launches and translation work differ. A score that only correlates with native target-cache MSE is not deployable because that cache requires the prefill being avoided.
...
- The commutator score does not beat transferred-state confidence or target-token-matched recent replay.
