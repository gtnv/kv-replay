...
from kv_research.artifacts import sha256_file
...
for entry in entries:
    ...
    for relation in ("parents", "supersedes"):
        ...
        if unknown:
            raise RuntimeError(
                f"ledger line {line_number} {relation} references later or missing entries {sorted(unknown)}"
            )
    ...
    if entry["outcome"] in ("success", "failure"):
        ...
        if artifact_dir not in audit_path.parents or sha256_file(audit_path) != entry["audit_sha256"]:
