...
elif manifest["status"] not in ("complete", "failed_control"):
...
if "online" in records[0]:
    ...
    recomputed["primary_native_control_passed"] = recomputed["conditions"][primary_replay][
    ...
    identity_continuation_bound = (
        config["identity_continuation_bound"]
        if "identity_continuation_bound" in config
        else "machine_floor"
    )
    recomputed = control_summary(
        records,
        config["native_cache_nmse_limit"],
        identity_continuation_bound,
    )
    expected_status = "complete" if recomputed["passed"] else "failed_control"
    if manifest["status"] != expected_status:
        raise RuntimeError(
            f"control status {manifest['status']} disagrees with recomputed {expected_status}"
        )
...
result = {
    "run": str(run_dir.relative_to(run_dir.parents[1])),
...
if args.output:
    ...
    if run_dir not in output_path.parents:
        raise RuntimeError("audit output path escapes run directory")
