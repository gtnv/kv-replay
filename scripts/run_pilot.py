...
def evaluate_chunks(chunks, partition, replay_lengths, source_model, target_model, translation):
    ...
    for index, chunk in enumerate(chunks):
        ...
        # Native target state exists only for offline labels and numerical controls;
        # every deployable score below is computed before it is placed in the record.
        ...
        for replay_length in replay_lengths:
            ...
            # The probe reaches the same anchor after translating an older prefix.
            # Disagreement with full transfer measures transition-inconsistent error.
...
def policy_action(
    ...
    if score <= threshold:
        ...
        return probe_result
    # Rejection is deliberately a fresh target prefill. This makes the timing
    # result an accept/fallback policy cost, not only the cost of computing a score.
    ...
    for replay_length in config["replay_lengths"]:
        for score_name in summary["score_names"]:
            for label_name in ("full_transfer_top1_diff", "replay_top1_diff"):
                ...
                # Accepted and rejected branches have different work. Timing both
                # and weighting by untouched-test coverage preserves that asymmetry.
...
primary_replay = str(config["primary_analysis"]["replay_length"])
summary["primary_native_control_passed"] = summary["conditions"][primary_replay][
    "native_chunk_control"
]["passed"]
if not summary["primary_native_control_passed"]:
...
console_summary["primary_native_control_passed"] = summary["primary_native_control_passed"]
